...
% Structural labels in a column beside the examples, with spoken forms:
% "[CP]" is an abbreviation a screen reader can neither expand nor usefully
% spell, so \SetAnnotSpoken registers what it should say and the annotation
% reaches the tree as a Span carrying /Alt.  The page still shows [CP], and
% copy-and-paste still yields it.  One of the two is glossed, because a gloss
% annotation is emitted from inside the grid's own paragraph rather than from
% the example's -- a different code path with the same /Alt to carry.
\SetAnnotSpoken{[CP]}{complementizer phrase}
\SetAnnotSpoken{[TP]}{tense phrase}
{\setlength{\ExAnnotColumn}{9cm}
\ex.
\a. que Pierre est fatigué\exannot{[CP]}
\b. Pierre est fatigué\exannot{[TP]}
\z.

\exg. que Pierre est fatigué \exannot{[CP]}\\
      that Pierre is tired\\
\glt `that Pierre is tired'}

% The same, with the column measured from the examples instead of declared
% (\ExAnnotFit).  Here for the veraPDF gate rather than for the geometry:
% the measurement hangs a \write whatsit inside the example's own paragraph,
% i.e. inside marked content the tagging code has open, and that this does
% not disturb the structure is exactly the kind of thing only the validator
% can say.  Group-scoped, so the rest of the document is unaffected.
{\ExAnnotFit
\ex.
\a. que Pierre est fatigué\exannot{[CP]}
\b. Pierre est fatigué\exannot{[TP]}
...
% The free translation set BESIDE the grid rather than under it.  Here for
% the veraPDF gate: the grid and the translation are two boxes, each with
% marked content of its own, put down inside a paragraph that has already
% opened some -- and nesting them there is what every veraPDF profile calls
% compliant while its log does not.  Reading order stays grid-then-
% translation, which is what a screen reader needs.
{\GlossTransSide
\exg. il mio libro\\ the my book\\
\glt `my book, with a translation given a column of its own so that the
      gloss above it need not give up the width.'}
