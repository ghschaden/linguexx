...
replace. \pkg{linguexx} is a reimplementation from the ground up,
undertaken for two reasons. The first is to repair the structural
weaknesses of the original: a globally drifting sub-example depth
counter, a judgment tokenizer built on catcode manipulation, and a
termination rule that failed at the end of a \pkg{beamer} frame. All
three are gone. The second is to bring the glossing closer to the
...
The package also provides an implementation of linguistic glossing
that is fully compatible with tagged pdfs, and produces valid output
according to \pkg{veraPDF}.
...
ancestor \pkg{linguex}: \cs{alt}, \cs{altg}, and \cs{exsource}.
...
juts out), \cs{AltBraceSep} (\texttt{0.15em}, the gap between brace and
stack), \cs{AltBraceOuterSep} (\texttt{0.7em}, the gap between the brace
and whatever precedes or follows it --- wider than \cs{AltBraceSep}
because the drawn curl bulges outward past its own box only on that
side), and \cs{AltBraceRaise} (\texttt{0pt}, a vertical nudge) ---
...
alternatives (\cs{alt}) are set in text mode rather than as a formula,
