...
to the left of an \cs{altn} stack instead of displacing the alternative it
marks (section~\ref{sec:altn}). Alternatives stacked with \cs{altg} are not
covered: that stack is written once per tier and the tiers have to keep the
same width, which a gutter on one tier alone would break.
...
widths the column takes the widest and every mark sits flush against the
words --- the same rule a judgment on a whole example follows. A stack in
which no alternative carries a mark is set exactly as it would be with the
option off, so turning the option on never moves a stack that has nothing
to align.
