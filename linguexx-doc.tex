...
% A manual about macros is mostly prose carrying unbreakable \texttt runs:
% \cs{DeclareJudgment}\marg{command}\marg{mark} is one ~200pt token with no
% hyphenation point in it.  When such a run does not fit, TeX weighs a very
% loose preceding line against an overfull one and, with the default zero
% emergency stretch, takes the overfull -- six of them, the worst 26pt (about
% 9mm) into the right margin of a printed A4 page.  A third pass over looser
% interword space is the cheaper trade here, and it is the standard remedy:
% nothing is reworded and no markup changes meaning.  Keep an eye on the box
% count (`lxx log linguexx-doc.log`) rather than on this number.
...
% The \penalty is what lets \cs{Name}\marg{a}\marg{b} break at all: without
% it the whole signature is one ~200pt token with no breakpoint, and when it
% will not fit TeX overruns the margin instead.  300 discourages the break
% without forbidding it, so a signature still prefers to stay whole and
% splits only when the alternative is ink in the margin.  Rewording the
% sentences instead would fix the two that overflow today and nothing about
% the next one.
\newcommand{\marg}[1]{\penalty300 \texttt{\{}\meta{#1}\texttt{\}}}
\newcommand{\oarg}[1]{\penalty300 \texttt{[}\meta{#1}\texttt{]}}
...
\paragraph{\pkg{leipzig} (Natalie Weber).} The table of Leipzig glossing
abbreviations behind \cs{lpzg} (section~\ref{sec:leipzig}) follows Natalie
Weber's \pkg{leipzig}, to which it owes both the list and the idea of
treating the abbreviations as a resource the document can be asked about.
...
\paragraph{One of them, not both.} \pkg{linguexx} replaces these packages
rather than accompanying them, and a document must load only one.
\pkg{linguex}, \pkg{expex}, \pkg{gb4e} and \pkg{langsci-gb4e} all define
\cs{ex} --- as does this one, and all of them with \cs{def}, so whichever
file is read second wins and nothing says so. The result compiles: the
examples go on numbering, and what breaks is a detail in the middle of the
document. \pkg{linguexx} therefore stops with an error naming the other
package, whichever order they were loaded in. The environment and
\cs{ea} syntaxes are available here as the options \opt{gb4e} and
\opt{langsci} (sections~\ref{sec:envs} and~\ref{sec:langsci}), which is
what a document that wants them should use. One case cannot be caught
this way: \pkg{expex} is plain \TeX{} and may be read with \cs{input},
which leaves no record of a package having been loaded. There the
package warns that \cs{ex} is no longer its own and leaves the document
alone.
...
\pkg{langsci-gb4e} \emph{does}, which is what a real source relies on;
the list below is accordingly generous. Two names are not provided --- see
\emph{What this package does not provide} below. Everything here is
...
do what they do in \pkg{langsci-gb4e} --- suppress \cs{jambox}, stop the label box from
...
\item \cs{exp} keeps \emph{both} meanings. It is the \LaTeX\ kernel's math
  operator as well as \pkg{langsci-gb4e}'s item command, and here the mode
  decides between them: inside math it is the operator, outside it is the
  item. A document may use both freely.
\item \cs{exp}'s prime and \cs{atcenter} are written in \emph{text mode}.
  \pkg{langsci-gb4e} writes them \verb|$'$| and \verb|$\vcenter{...}$|, and math here
  would put a \texttt{Formula} element in the structure tree, which costs a
  document its PDF/UA-2 conformance (section~\ref{sec:tagging}). \cs{atcenter} therefore centres on
...
\item \cs{eas} boxes its example in a \texttt{minipage} and not in a
  \texttt{tabular}, so that the example's prose is not wrapped in a
  \texttt{Table} element --- a screen reader would announce it as a table,
  and PDF/UA does not allow it. A footnote inside one
...
\item \cs{examplesroman} and \cs{examplesitalics} set the object tier and
  not the example font. Setting the object tier is what they are wanted for
  in a glossed example, and it is what they do here; the example font is
  \cs{exfont}'s to set.
...
\item \opt{nocgloss} is accepted and reports. In \pkg{langsci-gb4e} it stops
  a bundled copy of \pkg{cgloss} from being read so that another package can
  own \cs{gll}; here there is no bundled copy to withhold, and the glossing is
...
\texttt{xlistabr} and \texttt{qlist} are not provided. They are not silently
absent either: both names are defined here, and defined to stop at the line
that uses them with a message naming what to write instead ---
\texttt{xlist} for the first, an ordinary \LaTeX\ list for the second. An
error at the line that has to change is friendlier than a surprise further
down the page.
...
\afterex
The mark must come first in the item, \emph{before the example text}. A
\cs{label} or a \cs{sublabel} may precede it: they make no ink, so the mark
is still the first thing on the line and is still hung in the margin.
...
\afterex
Anything that does print --- a \cs{textbf}, an opening bracket, a footnote
mark --- ends the recognition, and a mark behind it stays in the text where
it was typed. Use \cs{jdg} (section~\ref{sec:jdgmanual}) for those, and for
marks outside the four characters below.

The conventional readings are the usual ones (\texttt{*}
...
\subsection{Arbitrary marks: \cs{jdg}}\label{sec:jdgmanual}
...
\noindent The padding is an invisible box the width of the leading marks
\emph{set in the object-line font}, so alignment holds even when the gloss
tier is set at a different size (a \cs{footnotesize} gloss still lines up
under the full-size bracket). It ships no ink and no marked content, so
tagging is unaffected. Any number of leading marks are skipped together; the
marks that count are, by default, the judgment marks \verb|* ? \# \%| and the
...
A translation usually keeps the delimiters of what it translates, and such a
gloss word is padded only by the \emph{difference}. In

\begin{code}
{\GlossPhantomAlign
\ex. \gll della sua morte (\`e questa)\\
of.the his death (is this.one)\\
}
\end{code}

\rendered
{\GlossPhantomAlign
\ex. \gll della sua morte (\`e questa)\\ of.the his death (is this.one)\\}

\noindent the gloss carries a parenthesis of its own, so its first real glyph
already sits where it belongs and nothing is added: the two parentheses stay
flush. Were the tier set at a smaller size its parenthesis would be narrower
than the object's, and the pad would be exactly that difference --- enough to
put the real glyphs under one another, and no more. Should the gloss open
with \emph{more} mark than the object word does, aligning the real glyphs
would mean pulling the gloss to the \emph{left} of its column, into the space
belonging to the word before it; that is not done. The gloss keeps the column
and its extra mark hangs out on the left.
...
There is one restriction on what may be a mark, and it is worth stating as a
rule: \emph{math-mode code typed directly into the example cannot be a
delimiter; a command that contains math can.} A Quine corner written
\verb|$\ulcorner{}$| in the object word cannot be declared, however you
spell the declaration. The same corner given a name --- \cs{qq}, defined as
\verb|\ensuremath{\ulcorner}| --- can be, and from then on behaves like
every other mark in the set.

The reason is \emph{one token is one mark} read the other way round. The
scan looks at the first \emph{token} of the word, and in
\verb|($\ulcorner{}$machin| that token is the \texttt{\$}, which is not a
mark; the scan stops there, before it has so much as seen \cs{ulcorner}. So
putting \cs{ulcorner} in the set changes nothing, and says nothing --- the
gloss simply goes on being padded as though the corner were part of the
word. \verb|\ensuremath{\ulcorner}| written out in the example fails
identically, its first token being \cs{ensuremath}. Nor is declaring the
\texttt{\$} itself a way round it: the scan would then peel the dollar and
the corner off and stop at the empty group between them, leaving a leading
run with its math unbalanced --- \texttt{! Extra \}, or forgotten \$}, at
the end of the example. A name is the only thing here with a single token
to offer:
...
\ex. \gll AAA (\qq{}machin BBB\\
     aaa \qq{}thingy bbb\\
\end{code}

\noindent The empty group after \cs{qq} does two jobs. It keeps the control
word off the letters that follow it without using a \emph{space} --- a space
there would end the word, and \cs{gll} would build two columns out of the
one --- and the scan stops at a brace group, so the leading run comes out as
exactly \verb|(\qq| and no more.

Without the declaration that example still lines up, as it happens: the
corner is then simply part of the word, in the object line and in the gloss
alike, and the parenthesis alone accounts for the difference between them.
Declaring it earns its keep when the gloss does \emph{not} carry the corner
--- then \texttt{thingy} sits under \texttt{machin}, where undeclared it
would sit under the corner.
...
settable for one example or for the whole document. \cs{GlossTransRightSkip}
(default \texttt{0pt plus 2em}) is a little stretch at the right of the
translation's column: a narrow measure hands \TeX\ lines it cannot break any
shorter, and a fully justified column then overflows rather than ending
short. All three defaults follow \pkg{expex}, so that one example set with
either package comes out in much the same proportions. The gloss is what is
being read closely, and it gets the room.
...
with room for it, and says so as an overfull \verb|\vbox| if it does not
get one.
...
running prose does, so the \texttt{.6} default leaves this one cramped,
while too generous a share leaves the translation's last line a single
word. There is no formula: try two or three values and look at the result.
That is a fair description of the whole feature --- it is a typesetting
decision, and the parameter is there so that one example can make it
...
\texttt{/Alt} is used rather than \texttt{/E} or \texttt{/ActualText}
because it is the one of the three that leaves both the printed label and
the text layer alone; \texttt{doc/TAGGING-NOTES.md} sets out the comparison
for anyone auditing a document's structure.
...
\paragraph{And \cs{jambox}.} Under \opt{langsci} this package also provides
Alexis Dimitriadis's \cs{jambox} (\S\ref{sec:langsci}), which answers a
neighbouring question: it lines material up a fixed distance from the
\emph{right} margin, and for a note that belongs near the margin it is the
more direct instrument of the two. \cs{exannot} is aimed at the other case
--- a column set close to the examples, where what has to be held is the
column's position rather than its distance from the margin --- and the two
are built differently because of it. \cs{jambox} is provided exactly as
\pkg{langsci-gb4e} has it; use whichever suits the note you are writing.
...
\begin{longtable}{@{}>{\ttfamily}l l p{0.44\linewidth}@{}}
\toprule
...
\textbackslash GlossTransSep & \texttt{2em} &
  ...
  factor) is the share of the measure the gloss gets,
  \cs{GlossTransMinWidth} (\texttt{6em}) the width below which the side
  position is abandoned, and \cs{GlossTransRightSkip}
  (\texttt{0pt plus 2em}) the stretch at the right of the translation's
  column. \\
...
The number of a top-level example is bracketed by
\cs{ExLBr}/\cs{ExRBr}, and a footnote example's by
\cs{FnExLBr}/\cs{FnExRBr}. These are \pkg{linguex}'s names and
\pkg{linguex}'s meaning: \verb|\renewcommand{\ExLBr}{[}| together with
\verb|\renewcommand{\ExRBr}{]}| prints ``[1]'' in the label and in every
reference to it, and the footnote series keeps its parentheses until its
own pair is moved as well. The package reads all four through
\cs{theExLBr} and its relatives, which add the parenthesis suppression
that \cs{ExBareRefs} asks for (section~\ref{sec:refs}); redefine one of
\emph{those} and you replace the switch along with the character.
...
\subsection{Stacked alternatives, and the word between them}
\label{sec:altspoken}
A stack set by \cs{altn} or \cs{altg} is a vertical arrangement on the
page, and a screen reader cannot see that the rows are alternatives rather
than lines. Under active tagging \pkg{linguexx} therefore gives the stack
an alternate text that says so in words: \cs{altn}\marg{aa}\marg{bb} is
announced ``aa or bb'', and three alternatives become ``aa, bb, or~cc''.

That word is English, and so is the punctuation convention around it ---
English puts a comma before the final connector and most other languages
do not. \cs{SetAltSpoken}\marg{word} replaces the connector:
...
\noindent Write the word bare, without spaces: the spacing around it is
the package's business, so \verb|\SetAltSpoken{ou}| and
\verb|\SetAltSpoken{ ou }| are the same setting. Dropping the comma before
the connector is part of the same change, because that is what these
languages do.

To keep it --- the English convention, and the package's default --- use
the starred form. An optional argument replaces the punctuation itself:
...
\noindent The setting respects grouping, like the \pkg{xlist} variants and
unlike \cs{SetLeipzig}, so one example may be spoken in another language
...
\noindent It applies to \cs{altg} as well as \cs{altn}, and as with
everything in this section it changes nothing on the page and nothing at
all in a document compiled without active tagging.
...
% ===========================================================================
\subsection{What this does not yet claim}\label{sec:tagginglimits}

The structure this package writes is validated, and validation is not the
same as knowing that a document reads well. It is worth being explicit about
the distance between the two.

\paragraph{No screen reader has been in the room.} veraPDF can say that a
file conforms to PDF/UA-2. It cannot say that a reader using JAWS, NVDA,
VoiceOver or Orca gets a good account of an interlinear gloss. Nothing here
has been tried with one. The structural decisions are argued rather than
observed --- above all the decision to read a gloss word bundle by word
bundle, so that an object word and its gloss are heard together, instead of
the whole object line and then the whole gloss line. That is the choice this
package would defend, but it is a choice, and only a user can settle it.
Treat this part of the package as a well-formed proof of concept: correct
by every check that can be automated, and not yet confirmed by the only
check that counts.

\paragraph{The spoken forms are English.} Three things carry words rather
than markup, and all three are written in English:

\begin{itemize}[leftmargin=*]
\item the judgment phrases --- ``ungrammatical'', ``questionable'' and the
  rest (section~\ref{sec:jdgspoken});
\item the Leipzig expansions --- ``singular'', ``accusative'' and some two
  hundred others (section~\ref{sec:leipzig});
\item the word \cs{altn} puts between stacked alternatives when it builds
  their spoken form: ``A, B, or C''.
\end{itemize}

\noindent
The first two can be replaced entry by entry, with \cs{SetJudgmentSpoken}
and \cs{SetLeipzig}, and the third with \cs{SetAltSpoken}
(section~\ref{sec:altspoken}).

So setting \verb|lang=fr| in \cs{DocumentMetadata} is not enough to make a
French document read properly. The tree would then declare French while its
\texttt{/Alt} and \texttt{/E} strings remained English words, and a French
voice would pronounce them as French. Nor is there a way to mark them as
English in passing: the \texttt{Span} that carries an \texttt{/Alt} does not
also carry a \texttt{/Lang}, so the phrase is announced in whatever language
surrounds it.

Calling that a translation job would be too quick, because for the gloss
abbreviations it is not clear what should be translated. The printed label
is already free: \cs{lpzg} small-caps whatever you type and never consults
the table, and keys are compared with their accents normalised, so
\verb|\lpzg{fém}| prints \textsc{fém} today, and
\verb|\SetLeipzig{f\'em}{féminin}| gives it an expansion although it spells
the key the other way. A French
document can be glossed in French, entry by entry, right now. What is
undecided is what the package should \emph{ship}: whether the Leipzig
abbreviations stay in place in every document --- they are English-derived
but serve as an interchange standard well beyond English --- with only the
spoken expansions translated, or whether a language should have its own
labels as well. The first keeps a glossed example legible to any reader of
the field, and costs the link between the label and the word spoken for it.
The second is more coherent inside one document, and gives up that
portability. This manual does not answer the question, and the package does
not answer it by default; \texttt{doc/DEFERRED-DECISIONS.md} records what
would settle it.

\paragraph{The kernel's tagging is still provisional.} These features sit on
the \LaTeX\ Project's tagging code at \texttt{phase-III}, whose interfaces
are declared provisional by the project itself. Where this package leans on
an internal it does so behind a guard that degrades to no tagging rather
than to a broken document, but the arrangement will want revisiting as that
work moves out of testphase.

\paragraph{What is checked, and how often.} One accessible configuration
is validated with veraPDF on every run of the test suite, and the worked
accessible document shipped in \texttt{examples/} before a release. The
number of
combinations a document can present --- syntax option, gloss shape, beamer,
footnotes, a language per tier --- is much larger than the set under
validation. Support for \texttt{/E} and \texttt{/Alt} also varies between
readers; a reader that ignores \texttt{/E} will simply say ``sg''.

If you use this with a screen reader, reports of what it actually sounds
like are very welcome. That is the missing evidence, and no amount of
validation substitutes for it.
...
\item \textbf{Examples in a \texttt{minipage} footnote.} They are numbered
  ...
  question none of \pkg{linguex}, \pkg{gb4e} and \pkg{langsci-gb4e} settles
...
\item \textbf{A stack inside a stack, under tagging.} \cs{altn} may be
  written inside an alternative of another \cs{altn}, and the page is
  right: both stacks are drawn and braced, the inner one set inside the
  outer's row. What is not right is the \emph{spoken} form. The outer
  stack's alternative text is built from the source of its alternatives,
  so a nested call contributes its own bracketed option and its
  alternatives run together with no connector between them; and because
  the inner stack is built before the outer's element is opened, the two
  come out as siblings rather than one inside the other, so a screen
  reader reaches the inner stack first and announces its alternatives
  twice. Nothing is claimed here and nothing is patched: what a nested
  stack ought to \emph{say} is undecided, and guessing would turn the
  guess into the promise. If the example is meant to be read aloud, write
  it as two examples.
...
These are not style advice but constraints the engine relies on. Each can
be broken without any error being raised, and the output then looks
plausible, which is what makes them worth stating.
...
\textbackslash GlossTransSide & Set the free translation in a column \emph{beside} the gloss rather than under it (section~\ref{sec:transside}); \cs{GlossTransBelow} restores the default. A declaration, and it must come before the example. Top-level examples only, and not on a gloss carrying an \cs{exannot}. \\
\textbackslash GlossTransRatio & Share of the measure the gloss gets beside a side translation (\texttt{.6}, a factor, not a length). \\
\textbackslash GlossTransSep & Gap between the two columns (\texttt{2em}). \\
\textbackslash GlossTransRightSkip & Stretch at the right of the translation's column (\texttt{0pt plus 2em}); without it a narrow column overflows rather than ending short. \\
\textbackslash GlossTransMinWidth & Below this the side position is abandoned and the translation set underneath, with a warning (\texttt{6em}). \\
...
\textbackslash SetAltSpoken\marg{word} & The word spoken between stacked alternatives, for \cs{altn} and \cs{altg} alike: \cs{SetAltSpoken}\marg{ou} gives ``aa, bb ou cc''. The starred form keeps the comma before the connector, which is English usage and the default; an optional \oarg{punct} replaces the punctuation. Respects grouping (section~\ref{sec:altspoken}). \\
\textbackslash exsource\marg{text} & Flush-right attribution. \\
\textbackslash exannot\oarg{spoken}\marg{text} & Structural label, set in a column at \cs{ExAnnotColumn}; on the object line in a gloss. \cs{SetAnnotSpoken}\marg{text}\marg{phrase} registers a spoken form for a label used throughout (section~\ref{sec:exannot}). \\
\textbackslash ExAnnotFit & Measure that column from the examples instead: one column per example, past the longest of them. Costs an \texttt{.aux} round trip; \cs{ExAnnotNoFit} turns it off again. \\
\textbackslash ExAnnotColumn & Where the column is, from the \emph{left} edge of the text block (\texttt{.75\textbackslash columnwidth}). \\
\textbackslash ExAnnotSep & Least gap between an example and that column (\texttt{1em}); give it no stretch or shrink. \\
\textbackslash ExAnnotFont & Font of the annotation (\cs{normalfont}). \\
\addlinespace
