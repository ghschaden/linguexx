...
The option carries the rest of that package's \emph{documented} surface, so
that a preamble and a document body being ported need no editing.
``Documented'' means named in one of its two authorities: the LangSci author
guidelines (\S5.7.1 and the showcases of \S12), and the \pkg{langsci-gb4e}
manual. The distinction matters, because \pkg{langsci-gb4e} defines about two
dozen further commands that appear in neither --- see below.
...
\midrule
\endhead
\multicolumn{2}{@{}l}{\normalfont\itshape what the guidelines teach}\\
\textbackslash ea \dots\ \textbackslash z & an example, and its levels\\
\textbackslash ex & the next item at the level open\\
\textbackslash gll, \textbackslash glt & a two-tier gloss and its translation\\
...
\addlinespace
\multicolumn{2}{@{}l}{\normalfont\itshape what the \pkg{langsci-gb4e} manual adds}\\
\textbackslash glll \dots\ \textbackslash gllllllll & three to eight gloss tiers (\S8.2)\\
\texttt{exe}, \texttt{xlist} & the environment forms (\S6)\\
\textbackslash exi\marg{label} & an item with the label given, stepping no counter (\S7.1)\\
\textbackslash exr\marg{key} & an item labelled with another example's number (\S7.3)\\
\textbackslash exp\marg{key} & the same, primed (\S7.2)\\
\textbackslash sn & an unnumbered item (\S7.5)\\
\textbackslash xref\marg{key}, \textbackslash xxref\marg{key}\marg{key} & a reference, and a range in one pair of parentheses (\S7.4)\\
\textbackslash exewidth\marg{sample} & reserve a label box as wide as \meta{sample} (\S7.6)\\
\textbackslash judgewidth\marg{len} & no effect here; see below (\S7.6)\\
\textbackslash zlast & \textbackslash z, without the space below (\S7.6)\\
\textbackslash eanoraggedright & \textbackslash ea, allowing hyphenation (\S7.6)\\
\textbackslash exfont, \textbackslash exnrfont & the example body, and its number (\S11)\\
\textbackslash glossfont, \textbackslash transfont & the gloss tiers, and the translation (\S11)\\
\textbackslash fnexfont \dots & the same four inside a footnote (\S11)\\
\textbackslash nogltOffset,\linebreak \textbackslash resetgltOffset & the space above the translation (\S11)\\
\bottomrule
\end{longtable}
\end{center}
...
\pkg{langsci-gb4e} also defines a further two dozen commands that neither the
guidelines nor its own manual mentions:

\begin{itemize}[leftmargin=*,itemsep=2pt]
\item \cs{eal}, \cs{zl}, \cs{eas}, \cs{zs}, \cs{zllast}, \cs{eafirst},
  \cs{ealnoraggedright};
\item the six sub-list numbering variants \texttt{xlista},
  \texttt{xlistabr}, \texttt{xlisti}, \texttt{xlistn}, \texttt{xlistA},
  \texttt{xlistI}, and \texttt{qlist};
\item \cs{examplesroman}, \cs{examplesitalics};
\item \cs{attop}, \cs{atcenter}, \cs{xbox}, \cs{nobreakbox};
\item \cs{gblabelsep}, \cs{subexsep}, \cs{singlegloss}, \cs{nosinglegloss},
  \cs{twodigitexamples} and its two siblings;
\item and the four package options \opt{nojambox}, \opt{manualexewidth},
  \opt{lowerpenalty}, \opt{nocgloss}.
\end{itemize}

None of these is provided. Two of them could not honestly be: \texttt{xlistabr}
labels every item \texttt{(xnumii.} in \pkg{langsci-gb4e} and \texttt{qlist}
raises ``No counter `xnum' defined'' there, so neither has ever worked in a
released version --- implementing them here would not be compatibility but the
invention of a meaning for a name with no working precedent. The rest are
left out because the option's promise is the \emph{documented} interface, and
a smaller surface that is entirely real is worth more than a larger one that
is half hearsay.

They are not silently absent, though. Every one of them is defined to raise an
error naming what to write instead, so a document being ported stops at the
line that has to change rather than at ``Undefined control sequence'':

\begin{code}
! Package linguexx Error: \eal is not part of the documented
                          langsci-gb4e interface.

[langsci] provides what the LangSci author guidelines and the
langsci-gb4e manual document, and \eal is in neither.
Write \ea, then a nested \ea for the list under it.
\end{code}

\noindent The four package options behave the same way but warn rather than
stop: a preamble is not yet typesetting anything, and a warning names the
situation without ending the run.

Upstream's automatic widening of the label box past 98 and 998 examples is not
provided either, for the same reason: its only interface is
\opt{manualexewidth}, which neither document mentions, and the manual's own
instruction for a document with three-digit numbers is to call
\cs{exewidth}\marg{sample}.

\subsubsection*{Where this differs from \pkg{langsci-gb4e}, and why}

Four things among what \emph{is} provided are not copies.
...
\item \cs{exp} keeps \emph{both} meanings. It is the \LaTeX\ kernel's math
  ...
  \verb|$\exp(x)|\verb|$| --- and, worse, stops compiling. Here the mode
  decides: inside math it is the operator, outside it is the item.
\item \cs{exp}'s prime is written in \emph{text mode}. Upstream writes it
  \verb|$'$|, and math here would put a \texttt{Formula} element in the
  structure tree --- the PDF/UA-2 failure that forced the removal of the old
  \cs{altg} in v0.12 (section~\ref{sec:tagging}). The mark is
  \cs{ExPrimeMark}, a raised \cs{textquotesingle}.
\item \cs{judgewidth} warns instead of acting. It sets the width of a
  reserved judgment column, and here a judgment hangs into the label gutter
  and reserves nothing --- which is exactly why marked and unmarked examples
  align (section~\ref{sec:jdgspace}).
\item The font commands are declarations, as upstream's are. The
  \pkg{langsci-gb4e} manual writes them with an argument
  (\cs{exfont}\marg{\cs{itshape}}), but its own \cs{exfont} takes none, so
  that spelling does nothing there either; use \cs{renewcommand} to set them.
\end{itemize}
