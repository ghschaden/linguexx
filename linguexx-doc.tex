...
and a document body being ported need no editing. Everything below is
...
\textbackslash begin\{xlistabr\} & letters in parentheses, \texttt{(a)}\\
...
\textbackslash begin\{qlist\} & a free-standing lettered list, margins symmetric\\
\addlinespace
...
Seven things are not copies, and each is worth knowing before a page is
...
\item \texttt{xlistabr} and \texttt{qlist} do what their names say.
  Upstream's \texttt{xlistabr} labels every item \texttt{(xnumii.} and
  upstream's \texttt{qlist} raises ``No counter `xnum' defined'' and labels
  every item ``\texttt{.}''; both are defects rather than conventions, so a
  document using them is not getting what it asked for today either.
