...
% \AltJdgTuck: a marked stack tucks toward its opening brace, into the
% hollow the brace's arm leaves.  Pinned by setting it to two known values
% and measuring the difference, so the assertion tests the mechanism and
% its sign rather than a number that moves with the default.  The stems are
% measured from their own sentinel, so the two stacks need not be adjacent.
{\renewcommand\AltJdgTuck{0pt}
\noindent TZEROL \lxAltn[l]{TZSTEM}{*TZSTEM} TZEROR\par}

% 5pt, not 6: past \AltBraceWidth + \AltBraceSep (5.91pt at this size)
% the package warns, and this case asserts elsewhere that it does not.
{\renewcommand\AltJdgTuck{5pt}
\noindent TFIVEL \lxAltn[l]{TSSTEM}{*TSSTEM} TFIVER\par}

% ... and an UNMARKED stack must not tuck at all, whatever the length says:
% the tuck exists to undo the gutter, and there is no gutter here.
{\renewcommand\AltJdgTuck{5pt}
\noindent UZEROL \lxAltn[l]{UZSTEM}{UZSTEM} UZEROR\par}
