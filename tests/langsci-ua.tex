...
%% invisibly.
%%
%% \exp is written $'$ upstream.  Math here means a Formula element, which is
%% the PDF/UA-2 failure that forced the 0.12 removal of the old \altg -- so
%% the tree must contain no Formula at all, and a_langsci_ua asserts exactly
%% that.  \exp must sit in an exe batch or after \ea; this one takes the
%% batch, which also gets the environment syntax into the tagged case.
\begin{exe}
\ex UALSVAR an example with a custom-labelled item under it.
\exp{uals:one} UALSPRIME a primed cross-reference as a label.
\ex UALSPLAIN a plain item after it.
\end{exe}
